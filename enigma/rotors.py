...
class Rotor(object):
  ...
  def __init__(self, wiring=None, notchs=None, name=None,
               model=None, date=None, state="A", ring="A"):
    """
  ...
  def __setattr__(self, name, value):
    ...
    if name == 'wiring':
      self.rwiring = ["0"] * 26
      for i in range(0, len(self.wiring)):
        self.rwiring[ord(self.wiring[i]) - ord('A')
                     ] = chr(ord('A') + i)
  ...
  def is_in_turnover_pos(self):
    return chr((ord(self.state) + 1 - ord('A')) %
               26 + ord('A')) in self.notchs
...
ROTOR_Gamma = Rotor(
  ...
  model="M4 R2",
  date="Spring 1941")
ROTOR_Reflector_A = Reflector(
  wiring="EJMZALYXVBWFCRQUONTSPIKHGD",
  name="Reflector A")
ROTOR_Reflector_B = Reflector(
  wiring="YRUHQSLDPXNGOKMIEBFZCWVJAT",
  name="Reflector B")
ROTOR_Reflector_C = Reflector(
